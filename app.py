...
# Custom Jinja2 filter for WAT display
def datetimeformat(value):
    try:
        dt = datetime.strptime(value, '%Y-%m-%d %H:%M:%S').replace(tzinfo=pytz.utc).astimezone(pytz.timezone('Africa/Lagos'))
        return dt.strftime('%b %d, %Y %H:%M WAT')
    except ValueError:
        return value
...
GITHUB_PATH = os.getenv("GITHUB_PATH", "re_bot.db")  # Updated to re_bot.db
...
USDT_AMOUNT = float(os.getenv("USDT_AMOUNT", 11.0))  # USDT amount for orders
...
# Database path
db_path = 're_bot.db'  # Updated to re_bot.db
...
# SQLite database setup with state persistence
def setup_database():
    global conn, live_position, live_buy_price, live_total_profit
    with db_lock:
        for attempt in range(3):
            try:
                ...
                if download_from_github('re_bot.db', db_path):  # Updated to re_bot.db
                ...
                c.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='state';")
                if not c.fetchone():
                    c.execute('''
                        CREATE TABLE state (
                            id INTEGER PRIMARY KEY AUTOINCREMENT,
                            live_position TEXT,
                            live_buy_price REAL,
                            live_total_profit REAL
                        )
                    ''')
                    logger.info("Created new state table")
                ...
                c.execute("SELECT live_position, live_buy_price, live_total_profit FROM state ORDER BY id DESC LIMIT 1")
                state = c.fetchone()
                if state:
                    live_position, live_buy_price, live_total_profit = state
                    logger.info(f"Restored live trading state: position={live_position}, buy_price={live_buy_price}, total_profit={live_total_profit}")
                conn.commit()
        ...
        try:
            if conn is None:
                ...
                if not setup_database():
                    logger.error("Failed to reinitialize database for state storage")
                    return
            c = conn.cursor()
            c.execute("INSERT INTO state (live_position, live_buy_price, live_total_profit) VALUES (?, ?, ?)",
                      (live_position, live_buy_price, live_total_profit))
            conn.commit()
            logger.debug(f"Saved live trading state: position={live_position}, buy_price={live_buy_price}, total_profit={live_total_profit}")
        except Exception as e:
...
def get_order_quantity(symbol, price, usdt_amount=USDT_AMOUNT):
    try:
        if symbol == 'BTC/USDT':
            quantity = 0.0001  # Default for BTC/USDT
        else:
            quantity = usdt_amount / price
        # Fetch Binance symbol info for LOT_SIZE filter
        ...
        for f in filters:
            if f.get('filterType') == 'LOT_SIZE':
                ...
                if step_size > 0:
                    ...
                    quantity = round(max(quantity, min_qty), precision)
                break
        logger.debug(f"Calculated order quantity for {symbol}: {quantity} at price {price}")
        return quantity
    except Exception as e:
        logger.error(f"Error calculating order quantity: {e}")
        return 0.0001 if symbol == 'BTC/USDT' else usdt_amount / price

# AI decision logic
def ai_decision(df, position=None):
    ...
    if position == "long":
        if close_price < open_price:
            logger.info(f"Sell condition met: close={close_price:.2f}, open={open_price:.2f}")
            action = "sell"
    elif position is None:
        if close_price > open_price and ema1 > ema2 and kdj_j > kdj_d:
            logger.info(f"Buy condition met: close={close_price:.2f}, open={open_price:.2f}, ema1={ema1:.2f}, ema2={ema2:.2f}, kdj_j={kdj_j:.2f}, kdj_d={kdj_d:.2f}")
            action = "buy"
...
def third_strategy(df, position=None, buy_price=None):
    ...
    if position == "long":
        if close_price < open_price:
            logger.info(f"Live sell condition met: close={close_price:.2f}, open={open_price:.2f}")
            action = "sell"
    elif position is None:
        if close_price > open_price and ema1 > ema2 and kdj_j > kdj_d:
            logger.info(f"Live buy condition met: close={close_price:.2f}, open={open_price:.2f}, ema1={ema1:.2f}, ema2={ema2:.2f}, kdj_j={kdj_j:.2f}, kdj_d={kdj_d:.2f}")
            action = "buy"
    ...
    if action in ["buy", "sell"] and bot_active:
        try:
            quantity = get_order_quantity(SYMBOL, close_price)
            if action == "buy":
    ...
    logger.debug(f"Third strategy: action={action}, order_id={order_id}")
            ...
            if conn is None:
                ...
                if not setup_database():
                    ...
                    return render_template('index.html', signal=None, status=status, timeframe=TIMEFRAME,
                                         trades=[], stop_time=stop_time_str, current_time=current_time, error="Failed to load database. Please try again later.")
            c = conn.cursor()
            c.execute("SELECT * FROM trades WHERE strategy IN ('primary', 'third') ORDER BY time DESC LIMIT 16")
            rows = c.fetchall()
            columns = [col[0] for col in c.description]
            trades = [dict(zip(columns, row)) for row in rows]
            signal = next((trade for trade in trades if trade['strategy'] == 'third'), trades[0] if trades else None)
            stop_time_str = stop_time.strftime("%Y-%m-%d %H:%M:%S")
            current_time = datetime.now(EU_TZ).strftime("%Y-%m-%d %H:%M:%S")
            elapsed = time.time() - start_time
            logger.info(f"Rendering index.html: status={status}, timeframe={TIMEFRAME}, trades={len(trades)}, signal_exists={signal is not None}, signal_time={signal['time'] if signal else 'None'}, signal_strategy={signal['strategy'] if signal else 'None'}, query_time={elapsed:.3f}s")
            return render_template('index.html', signal=signal, status=status, timeframe=TIMEFRAME,
                                 trades=trades, stop_time=stop_time_str, current_time=current_time, error=None)
        except Exception as e:
            elapsed = time.time() - start_time
            logger.error(f"Error rendering index.html after {elapsed:.3f}s: {e}", exc_info=True)
            conn = None
            stop_time_str = stop_time.strftime("%Y-%m-%d %H:%M:%S")
            current_time = datetime.now(EU_TZ).strftime("%Y-%m-%d %H:%M:%S")
            return render_template('index.html', signal=None, status=status, timeframe=TIMEFRAME,
...
def log_request():
    try:
        logger.debug(f"Incoming request: method={request.method}, path={request.path}, headers={request.headers}, data={request.get_data()[:100]}")
    except Exception as e:
...
async def main():
    pass  # Placeholder for async main function
