...
# Flask app setup
app = Flask(__name__)
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1, x_host=1, x_port=1)  # Trust proxy headers for HTTPS
        ...
        for attempt in range(3):
            try:
                ...
                if not c.fetchone():
                    ...
                    ''')
                    logger.info("Created new trades table with all columns")
                ...
                for col, col_type in required_columns.items():
                    if col not in columns:
                        ...
                        logger.info(f"Added column {col} ({col_type}) to trades table")
                conn.commit()
...
def ai_decision(df, stop_loss_percent=STOP_LOSS_PERCENT, take_profit_percent=TAKE_PROFIT_PERCENT, position=None, buy_price=None):
    ...
    elapsed = time.time() - start_time
    logger.debug(f"AI decision completed in {elapsed:.3f}s: action={action}, stop_loss={stop_loss}, take_profit={take_profit}, order_id={order_id}")
...
async def send_telegram_message(signal, bot_token, chat_id, retries=3, delay=5):
    start_time = time.time()
    for attempt in range(retries):
        try:
...
"""
            ...
            logger.error(f"Network error sending Telegram message (attempt {attempt + 1}/{retries}): {e}")
            if attempt < retries - 1:
                await asyncio.sleep(delay)
        except Exception as e:
            logger.error(f"Unexpected error sending Telegram message (attempt {attempt + 1}/{retries}): {e}", exc_info=True)
            if attempt < retries - 1:
                await asyncio.sleep(delay)
    elapsed = time.time() - start_time
...
async def handle_telegram_updates(application, last_update_id):
    while True:
        try:
            updates = await application.bot.get_updates(offset=last_update_id, timeout=5)
            for update in updates:
                if update.message and update.message.text:
                    text = update.message.text.strip()
                    command_chat_id = update.message.chat.id
                    if text == '/help':
                        await application.bot.send_message(chat_id=command_chat_id, text="Commands: /help, /stop, /stopN, /start, /status, /performance, /count")
                    elif text == '/stop':
                        with bot_lock:
                            if bot_active and position == "long":
                                ...
                                if not pd.isna(latest_data['Close']):
                                    ...
                                    signal = create_signal("sell", latest_data['Close'], latest_data, df, profit, total_profit, return_profit, total_return_profit, f"Bot stopped via Telegram{msg}", order_id, "primary")
                                    store_signal(signal)
                                    await application.bot.send_message(chat_id=CHAT_ID, text=signal['message'])
                                    position = None
                            bot_active = False
                        await application.bot.send_message(chat_id=command_chat_id, text="Bot stopped.")
                        upload_to_github(db_path, 're_bot.db')
                    elif text.startswith('/stop') and text[5:].isdigit():
                        multiplier = int(text[5:])
                        with bot_lock:
                            pause_duration = multiplier * TIMEFRAMES
                            ...
                            if position == "long":
                                ...
                                if not pd.isna(latest_data['Close']):
                                    ...
                                    try:
                                        order = exchange.create_market_sell_order(SYMBOL, quantity)
                                        order_id = str(order['id'])
                                        logger.info(f"Placed market sell order on /stopN: {order_id}, quantity={quantity}, price={latest_data['Close']:.2f}")
                                    except Exception as e:
                                        logger.error(f"Error placing market sell order on /stopN: {e}")
                                    signal = create_signal("sell", latest_data['Close'], latest_data, df, profit, total_profit, return_profit, total_return_profit, f"Bot paused via Telegram{msg}", order_id, "primary")
                                    store_signal(signal)
                                    await application.bot.send_message(chat_id=CHAT_ID, text=signal['message'])
                                    position = None
                            bot_active = False
                        await application.bot.send_message(chat_id=command_chat_id, text=f"Bot paused for {pause_duration/60} minutes.")
                        upload_to_github(db_path, 're_bot.db')
                    elif text == '/start':
                        with bot_lock:
                            if not bot_active:
                                ...
                                await application.bot.send_message(chat_id=command_chat_id, text="Bot started.")
                    elif text == '/status':
                        ...
                        await application.bot.send_message(chat_id=command_chat_id, text=status)
                    elif text == '/performance':
                        await application.bot.send_message(chat_id=command_chat_id, text=get_performance())
                    elif text == '/count':
                        await application.bot.send_message(chat_id=command_chat_id, text=get_trade_counts())
                last_update_id = update.update_id + 1
        except Exception as e:
            logger.error(f"Error processing Telegram updates: {e}", exc_info=True)
        await asyncio.sleep(1)

def start_telegram_updates():
    application = Application.builder().token(BOT_TOKEN).build()
    asyncio.run(handle_telegram_updates(application, last_update_id=0))

# Trading bot
def trading_bot():
    global bot_active, position, buy_price, total_profit, pause_duration, pause_start, conn, stop_time, df
    bot = None
    try:
        application = Application.builder().token(BOT_TOKEN).build()
        bot = application.bot
        ...
        asyncio.run(send_telegram_message(test_signal, BOT_TOKEN, CHAT_ID))
    ...
    telegram_thread = threading.Thread(target=start_telegram_updates, daemon=True)
    telegram_thread.start()
    logger.info("Telegram updates thread started")
            ...
            if STOP_AFTER_SECONDS > 0 and datetime.now(EU_TZ) >= stop_time:
                ...
                if position == "long":
                    ...
                    if not pd.isna(latest_data['Close']):
                        ...
                        store_signal(signal)
                        if bot:
                            asyncio.run(send_telegram_message(signal, BOT_TOKEN, CHAT_ID))
                    position = None
        ...
        try:
            if pause_start and pause_duration > 0:
                ...
                if elapsed < pause_duration:
                    ...
                    if bot:
                        asyncio.run(bot.send_message(chat_id=CHAT_ID, text="Bot resumed after pause."))
            ...
            with bot_lock:
                ...
                if bot_active and action != "hold" and bot:
                    threading.Thread(target=asyncio.run, args=(send_telegram_message(signal, BOT_TOKEN, CHAT_ID),), daemon=True).start()
